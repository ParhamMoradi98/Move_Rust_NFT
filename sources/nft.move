...
module overmind::NonFungibleToken {
    ...
    */
    fun init(ctx: &mut TxContext) {
        ...
        // Transfer the minter cap object to the mintercap owner
    ...
    */
    ...
    // Check if the payment is sufficent.
    assert!(coin::value<SUI>(payment_coin)>= 1000000000, EInsufficientPayment);

    // Take one SUI and transfer it to the mintercap object balance.
    let nft_fee = coin::split(payment_coin, 1000000000, ctx) ; 
    coin::put(&mut minter_cap.sales, nft_fee);
   

   // Change the type of image from vector to URL.
    let nft_url =  url::new_unsafe_from_bytes(nft_image);

    // Change the type of name from vector to String.
    let name = string::utf8(nft_name);

    // Change the type of description from vector to String.
    let description = string::utf8(nft_description);

    // Create new ID for the minted nft.
    let nft_id = object::new(ctx);

    // Create the nft
    let nft = NonFungibleToken{id : nft_id,name:name,description: description,image:nft_url};

    // Emit event when an NFT token is minted.
    ...
    // transfer the nft to the recepient.    
    ...
    */
    ...
    event::emit(NonFungibleTokenCombined {
        ...
        new_nft_id: *object::uid_as_inner(&new_id),
    });

    // Create the first NFT and remove it from the objects.
    let NonFungibleToken {id: id1, name: name1, description: _, image: _} = nft1;
    object::delete(id1);


    // Create the second NFT and remove it from the objects.
    let NonFungibleToken {id : id2,name:  name2, description: _, image: _} = nft2;
    object::delete(id2);

    // Generate a combined NFT description by concatenating the names of two NFTs.
    let combined_nft_description = string::utf8(b"Combined NFT of ") ;
    string::append(&mut combined_nft_description, name1) ;
    let test_string = string::utf8(b" and ");
    string::append(&mut combined_nft_description, test_string) ;
    string::append(&mut combined_nft_description, name2);
    let name_combined = string::utf8(b"");
    string::append(&mut name_combined, name1) ;
    let test_string = string::utf8(b" + ");
    string::append(&mut name_combined,test_string) ;
    string::append(&mut name_combined, name2);


    // Change the type of image from vector to URL.
    let nft_url =  url::new_unsafe_from_bytes(new_image_url);


    // Create the nft.
    let nft = NonFungibleToken{id: new_id,name:name_combined, description:combined_nft_description,image:nft_url};
    ...
    */
    ...
    // Retrieve the sales balance value
    let sales_value = sui::balance::value(&minter_cap.sales);

    // Create a new coin using the balance from the mintercap object
    let sold_coin = coin::take(&mut minter_cap.sales, sales_value, ctx);


    // Emit event when sales are withdrawn.
    event::emit(SalesWithdrawn{amount: sales_value});
    sold_coin
    }
    ...
    */
    ...
    // Emit event when token is burnt.
    event::emit(NonFungibleTokenDeleted{nft_id:  *object::uid_as_inner(&id) });

    // Remove NFT from the objects.
    object::delete(id);
